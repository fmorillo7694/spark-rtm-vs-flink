...
- **Confirmed at scale on EKS (~100k evt/s, all 5 engines).** A production cluster run
  ...
  **Flink owns the tail** (e2e p99 76 ms vs Spark RTM 183–233 ms) at **half the CPU/memory**;
...
- **Durable checkpointing flips the tail story decisively (§10).** With production-grade
  durable S3 checkpoints on, **Spark RTM's synchronous offset commit blows its p99 to
  ~1.1–1.4 s**, while Flink's async checkpoint holds p99 at ~50 ms — a **~20–25× gap**. On
  worker loss, both engines auto-recover from checkpoint in a comparable **~18–24 s** (an
  earlier "129 ms RTM recovery" of ours was a no-durable-checkpoint artifact and was
  discarded). This is the finding that most separates the two for real production use.
...
**~4 million** records over a 130 s steady-state window. Engines got identical envelopes
(Spark: 4 exec × 4 cores; Flink: 3 TM × 4 slots; parallelism 12).
...
- **Flink is the efficiency winner too:** 1.6 cores / 5 GB vs Spark RTM's ~3.4 cores / ~9 GB
  for the same throughput — roughly half the compute and memory.
...
|---|--:|--:|--:|
| **Flink** (Java) | 6.8 | **46** | 70 |
| **PyFlink** | 10.6 | **57** | 86 |
| **Spark RTM** (Java) | 8.1 | **1,319** | 1,480 |
| **Spark RTM** (Scala) | 7.9 | **1,063** | 1,200 |
| **PySpark RTM** | 8.0 | **933** | 1,040 |

_(All five rows above are from a clean re-run with empty topics and per-engine durable S3
checkpoints, each backed by a saved `results/eks/<engine>_summary.json`.)_

**This is the most important finding in the whole study, and it only appears once durable
checkpointing is on:**

- **Spark RTM's checkpoint commit is synchronous and stalls the data path.** Committing
  offsets to durable S3 every 5s produces a **~1.1–1.4 s p99 tail** across all three RTM
  variants. The median stays ~8 ms; the tail is wrecked.
- **Flink's checkpointing is asynchronous** and barely touches the data path — p99 stays at
  ~50–58 ms, essentially unchanged from the no-checkpoint run.
- So **durable fault tolerance costs Spark RTM ~20–25× worse tail latency than Flink.** If
  you turned off checkpointing to chase RTM's low tail (§9), you gave up recoverability.
- Java ≡ Scala holds here too (1,319 vs 1,063 ms — same synchronous-commit mechanism).
