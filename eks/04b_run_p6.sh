...
RUNID="${RUNID:-$(date +%s)}"; export CKPT_SUFFIX="-p6-${RUNID}"
mkdir -p results/eks
echo "==> $ENGINE p6: parallelism 6, 7 cores/14GB each, 60s ckpt, win=${RUN_SECONDS}s"
...
echo "==> [1/6] reset topics to 6 partitions"
kubectl delete kafkatopic input-events output-events -n kafka --ignore-not-found >/dev/null 2>&1; sleep 6
kubectl apply -f eks/p6_kafka_strimzi.yaml >/dev/null 2>&1; sleep 12

echo "==> [2/6] deploy $ENGINE (p6 manifest)"
case "$ENGINE" in
  spark-rtm-java)  render eks/jobs/p6/spark-rtm.yaml | sed "s#__MAINCLASS__#bench.RtmPipelineJava#;s#__JAR__#local:///opt/spark/jars/java-rtm.jar#;s#__TYPE__#Java#;s#__CKPTNAME__#spark-rtm-java${CKPT_SUFFIX}#" | kubectl apply -f - ;;
  spark-rtm-scala) render eks/jobs/p6/spark-rtm.yaml | sed "s#__MAINCLASS__#RtmPipeline#;s#__JAR__#local:///opt/spark/jars/scala-rtm.jar#;s#__TYPE__#Scala#;s#__CKPTNAME__#spark-rtm-scala${CKPT_SUFFIX}#" | kubectl apply -f - ;;
  pyspark-rtm)     render eks/jobs/p6/pyspark-rtm.yaml | kubectl apply -f - ;;
  flink)           render eks/jobs/p6/flink-datastream.yaml | kubectl apply -f - ;;
  flink-sql)       render eks/jobs/p6/flink-sql.yaml | kubectl apply -f - ;;
  pyflink)         render eks/jobs/p6/pyflink.yaml | kubectl apply -f - ;;
