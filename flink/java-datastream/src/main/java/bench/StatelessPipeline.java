...
public class StatelessPipeline {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Key the output record by user_id, matching the Spark job (which writes
        // key = CAST(user_id AS STRING)). A custom serializer pulls the key out of the
        // emitted "<key>\t<json>" pair so the partitioning is identical across engines.
        KafkaSink<String> sink = KafkaSink.<String>builder()
                .setBootstrapServers(bootstrap)
                .setRecordSerializer(new KeyedJsonSerializer(outputTopic))
    ...
    /**
     * Serializes the transform's "<user_id>\t<json>" output into a Kafka record keyed by
     * user_id (bytes) with the JSON document as the value — byte-for-byte the same keying
     * scheme as the Spark RTM job, so output partitioning is identical.
     */
    public static final class KeyedJsonSerializer
            implements KafkaRecordSerializationSchema<String> {
        private final String topic;

        public KeyedJsonSerializer(String topic) {
            this.topic = topic;
        }
        ...
        public ProducerRecord<byte[], byte[]> serialize(
            ...
            String key = element.substring(0, tab);
            String value = element.substring(tab + 1);
     ...
     * records as nothing (flatMap with 0 or 1 output). Output is "<user_id>\t<json>"
     * so the sink can key the Kafka record by user_id, matching Spark.
     */
    public static final class TransformFn
        ...
        public void flatMap(String value, org.apache.flink.util.Collector<String> outc)
            ...
            long userId = e.path("user_id").asLong();
            ...
            outc.collect(userId + "\t" + MAPPER.writeValueAsString(o));
        }
    }
}
