...
tolerance** across five engine variants — locally in Docker and on a production EKS
...
- RTM is real: it cuts median end-to-end latency from the micro-batch floor (~50 ms) to
  **~8 ms**, a 6.3× win.
- At the median RTM ties Flink (~7 ms). On the **tail**, Flink wins — 2–4× without durable
  checkpointing, and **~20–25× with it** (RTM's synchronous offset commit spikes p99 to
  ~1.3 s; Flink's async checkpoint stays ~50 ms).
- Flink does the same work on **~half the CPU and memory**.
...
## What's compared (5 engine variants)
...
| `flink` | Flink 2.2.0 DataStream, `bufferTimeout(0)` | Java 17 | the comparison engine |
...
Java-on-both. All run the same transform on the same event (see
...
bench/run_flink.sh              # Java DataStream            -> results/flink_*
